...
def generar_poliza(request):
    ...
    data['fecha_emision'] = request.POST.get('fecha_emision')
    ...
    data['prima_total'] = request.POST.get('prima_total')
    data['emision'] = request.POST.get('emision')
    data['iva'] = request.POST.get('iva')
    data['total_pagar'] = request.POST.get('total_pagar')
    data['forma_pago'] = request.POST.get('forma_pago')
    data['cuotas'] = request.POST.get('cuotas')
    data['valor_cuota'] = request.POST.get('valor_cuota')
    poliza = Poliza()
    poliza.fecha_emision = hoy
    # poliza.aseguradora = Aseguradora.objects.get(id=data['aseguradora'])
    # poliza.nombres = data['nombres']
    # poliza.apellidos = data['apellidos']
    # poliza.cedula = data['cedula']
    # poliza.telefono = data['telefono']
    # poliza.email = data['email']
    poliza.anno = data['anno']
    poliza.marca = data['marca']
    poliza.modelo = data['modelo']
    poliza.suma_asegurada = data['valor_depreciado']
    poliza.user = request.user
    poliza.chasis = data['chasis']
    poliza.motor = data['motor']
    poliza.circulacion = data['circulacion']
    # poliza.vin = data['vin']
    # poliza.certificado = data['certificado']
    # poliza.capacidad = data['capacidad']
    # poliza.tonelaje = data['tonelaje']
    poliza.placa = data['placa']
    poliza.color = data['color']
    # poliza.capacidad = data['capacidad']
    # poliza.tonelaje = data['tonelaje']
    poliza.uso = data['uso']
    poliza.valor_nuevo = data['valor_nuevo']
    poliza.forma_pago = data['forma_pago']
    poliza.subtotal = data['prima_total']
    poliza.emision = data['emision']
    poliza.iva = data['iva']
    poliza.total = data['total_pagar']
    poliza.cuotas = data['cuotas']
    poliza.monto_cuota = data['valor_cuota']
    # poliza.valor_depreciado = data['valor_depreciado']
    try:
        poliza.file_cedula = request.FILES['file_cedula']
    except:
        pass
    try:
        poliza.file_circulacion = request.FILES['file_circulacion']
    except:
        pass
    try:
        poliza.file_carta = request.FILES['file_carta']
    except:
        pass
    poliza.save()

    context = {'poliza': poliza}
    return render_to_pdf_response(request, 'cotizador/pdf/recibo.html', context)
...
#         try:
#             a = Auto.objects.get(poliza=p.no_poliza)
#             print("encontrada")
#             p.subtotal = a.prima
#             p.emision = round((a.prima - config.SOA_AUTOMOVIL) * 0.02, 2)
#             p.iva = round(((p.subtotal + p.emision) - config.SOA_AUTOMOVIL) * 0.15, 2)
#             p.total = round(p.subtotal + p.emision + p.iva, 2)
#             print("reparada")
